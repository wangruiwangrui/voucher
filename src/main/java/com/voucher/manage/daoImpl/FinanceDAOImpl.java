	...
	public Map findGeneralChartInfo(Integer limit, Integer offset, String sort, String order, Integer isHistory,
		...
		if (!search.isEmpty()) {
			...
			sql = " select top " + limit + " * from " + "(select ROW_NUMBER() OVER (ORDER BY " + sort + " " + order
					...
					+ ") as t2 on ChartInfo.GUID=t2.ChartGUID	" + "where ChartGUID is not null and  (" + serach + ")"
					+ ")as w1 " + "where rows>" + offset + " and " + "IsHistory=" + isHistory + " " + "order by " + sort
					+ " " + order;
...
//		int total = (int) SelectExe.getCount(this.getJdbcTemplate(), chartInfo).get("");
		...
		if (!search.isEmpty()) {
			...
			sql = "SELECT top ( "+ limit +" ) * FROM [ChartInfo] where GUID not in( select top "+ offset + " GUID FROM [ChartInfo] where "+serach
					+" ORDER BY ConcludeDate "+order+" ) AND ( "+serach+" ) ORDER BY ConcludeDate "+order;

			sql2 = "SELECT COUNT(*) FROM [ChartInfo] where "+serach;
		} else {

			sql = "SELECT top ("+limit+") * FROM  [ChartInfo] where GUID not in( select top "+offset+" GUID FROM [ChartInfo] ORDER BY ConcludeDate "+order+") ORDER BY ConcludeDate "+order;

			sql2 = "SELECT COUNT(*) FROM [ChartInfo]";
		}
